	      		...
	      		if (error.empty?)
					...
					courses.each_with_index { |x,i| 
						...
						parts.times { |x|
		            		...
		            		course = Course.find_or_create_by(course_id: unique_id) do |c|
		            			c.term = term
				            	c.subject = subject
				            	c.number = number
				            	c.section = section
		            		...
		            		if instructor == 'Staff'
		            			#Do something
		            			addProfandClass('Staff', 'Staff', unique_id)
		            		elsif instructor.include?(',')
		            			instructors = split(',')
		            			#Create new for each
		            			instructors.each do |ins|
							if ins == 2
		            					addProfandClass(ins[0], ins[1], unique_id)
		            				else
		            					Print "Incorrect Instructor Name type."
		            				end
		            			end

		            		else
		            			instr = instructor.split
		            			if instr == 2
		            				addProfandClass(instr[0], instr[1], unique_id)
		            			else
		            				Print "Incorrect Instructor Name type."
		            			end
		            		end

		            		def addProfandClass(fName, lName, uid)
		            			prof = Professor.find_or_create_by(firstName: fName, lastName: lName) do |p|
		            				p.course_ids = uid;
		            			end

		            		end

		            		#Specify instructor information 
